...
mod tests {
    ...
    fn streaming_matches_one_shot() {
        let mut analyzer = Analyzer::new();
        analyzer.add_file("src/main.rs", "// hi\nfn main() {\n    let x = 1;\n}\n");
        analyzer.add_file("src/lib.py", "# top\ndef f():\n    pass\n");
        analyzer.add_file("package.json", "{\n  \"name\": \"x\"\n}\n");
        analyzer.add_file("README.md", "# hi\n");
        let r: TestResult = serde_json::from_str(&analyzer.finalize()).unwrap();
